...
impl X11Context {
    ...
    pub fn get_mouse_pos(&self) -> (i32, i32) {
        unsafe {
            let (mut x, mut y) = (0, 0);
            (self.xlib.XQueryPointer)(self.display, self.root, &mut 0, &mut 0, &mut x, &mut y, &mut 0, &mut 0, &mut 0);
    ...
    pub fn create_window(&self, x: i32, y: i32, width: u32, height: u32) -> u64 {
        unsafe {
            let mut attributes: xlib::XSetWindowAttributes = MaybeUninit::uninit().assume_init();
    ...
    pub fn grab_keyboard(&self) {
        for _ in 0..1000 {
            if unsafe {(self.xlib.XGrabKeyboard)(
                self.display,
                self.root,
                xlib::True,
                xlib::GrabModeAsync,
    ...
    pub fn load_font(&self, font: &str) -> u64 {
        unsafe {
            (self.xlib.XLoadFont)(self.display, CString::new(font).unwrap().as_ptr())
        }
    }
    pub fn init_gc(&self, window: u64, font: u64) -> *mut xlib::_XGC {
        unsafe {
            let mut xgc_values: xlib::XGCValues = MaybeUninit::uninit().assume_init();
    ...
    pub fn run<F>(&self, mut handle_events: F)
        where F: FnMut(&Self, Option<&xlib::XEvent>) -> Action {
            let mut event: xlib::XEvent = unsafe { MaybeUninit::uninit().assume_init() };

            loop {
                if unsafe { (self.xlib.XCheckMaskEvent)(self.display, xlib::KeyPressMask, &mut event) } == 0 {
                    // no events available
                    ...
                    // we got some events
                    if handle_events(&self, Some(&event)) == Action::Stop {
                        break;
                    }
                }
            }
    ...
    pub fn draw_rect(&self, window: u64, gc: *mut xlib::_XGC, color: u64, x: i32, y: i32, width: u32, height: u32) {
        unsafe {
            (self.xlib.XSetForeground)(self.display, gc, color);
            (self.xlib.XFillRectangle)(self.display, window, gc, x, y, width, height);
        }
    }
    pub fn render_text(&self, window: u64, gc: *mut xlib::_XGC, color: u64, x: i32, y: i32, text: &str) {
        unsafe {
            (self.xlib.XSetForeground)(self.display, gc, color);
